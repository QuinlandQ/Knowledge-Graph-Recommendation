...
class PPKG_loader(Data):
    def __init__(self, args, path):
        ...
        self.batch_num = len(self._generate_train_lstm_batch())
    ...
    def _get_all_kg_data(self):
        ...
        for h in org_h_dict.keys():
            ...
            sort_t_list = reorder_list(org_t_list, sort_order)
            sort_r_list = reorder_list(org_r_list, sort_order)
            sort_v_list = reorder_list(org_v_list, sort_order)
    ...
    def _generate_train_lstm_batch(self):
        records_batch = []
        records_label = []

        def one_hot_label(record):
            y_labels = [0] * self.n_items
            y_labels[int(record)] = 1

            return y_labels

        records_for_u = self.generate__train_lstm_batch()

        exist_users = records_for_u.keys()
        user = rd.sample(exist_users, 1)

        for user in user:
            records_for_u = records_for_u[user]

        for i in range(len(records_for_u) + 1):
            if i < len(records_for_u) - self.lstm_input_size:
                record_batch = records_for_u[i:i+self.lstm_input_size]
                records_batch.append(record_batch)
                record_label = records_for_u[i + self.lstm_input_size]
                record_label = one_hot_label(record_label)
                records_label.append(record_label)

        return records_batch, records_label
    ...
    def generate_train_lstm_batch(self):
        records_batch, records_label = self._generate_train_lstm_batch()

        batch_data = {}
        batch_data['records'] = records_batch
        batch_data['labels'] = records_label

        return batch_data

    def generate_train_lstm_feed_dict(self, model, batch_data):

        feed_dict = {
            model.records: batch_data['records'],
    ...
    def generate_test_lstm_feed_dict(self, model, user_batch,  drop_flag=True):
        feed_dict ={
                    model.users: user_batch
                    }
    ...
    def generate_test_lstm__feed_dict(self, model, records_batch):
        feed_dict ={
                    model.records: records_batch
                    }
