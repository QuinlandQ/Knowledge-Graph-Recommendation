...
def load_pretrained_data(args):
    ...
    try:
        ...
        print('load the pretrained ppkg model parameters.')
    except Exception:
        pretrain_data = None
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.model_type in ['ppkg', 'cfkg']:
        ...
        config['records']  = data_generator.train_records
    ...
    *********************************************************
    ...
    elif args.model_type in ['ppkg']:
        model = PPKG(data_config=config, pretrain_data=pretrain_data, args=args)
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        ...
        elif args.model_type in ['ncf', 'nfm', 'ppkg']:
    ...
    *********************************************************
    ...
    if args.pretrain == 1:
        ...
        elif args.model_type in ['ncf', 'nfm']:
            layer = '-'.join([str(l) for l in eval(args.layer_size)])
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        *********************************************************
        ...
        """
            *********************************************************
             Training for LSTM:
            ... phase 3: to train the lstm_layer.
        """
        #tf.compat.v1.reset_default_graph()
        ...
        if args.model_type in ['ppkg']:
            ...
            if args.use_kge is True:
                lstm_batch_data = data_generator.generate_train_lstm_batch()
                lstm_feed_dict = data_generator.generate_train_lstm_feed_dict(model, lstm_batch_data)

                with tf.compat.v1.variable_scope('lstm',reuse=True):
                    _, batch_loss, batch_lstm_loss, batch_reg_loss = model.train_lstm(sess, feed_dict=lstm_feed_dict)

                loss += batch_loss
                kge_loss += batch_lstm_loss
                reg_loss += batch_reg_loss

        if np.isnan(loss) == True:
            print('ERROR: loss@phase3 is nan.')
            sys.exit()
    ...
    with open(save_path, 'a') as f:
        f.write('embed_size=%d, lr=%.4f, layer_size=%s, node_dropout=%s, mess_dropout=%s, regs=%s, input_size=%d, n_hidden_units=%d , layer_num=%d, alg_type=%s, adj_type=%s, use_att=%s, use_kge=%s, pretrain=%d\n\t%s\n'
            % (args.embed_size, args.lr, args.layer_size, args.node_dropout, args.mess_dropout, args.regs, args.input_size, args.n_hidden_units, args.layer_num, args.alg_type, args.adj_type, args.use_att, args.use_kge, args.pretrain, final_perf))
