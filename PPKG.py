...
class PPKG(object):
    ...
    def _parse_args(self, data_config, pretrain_data, args):
        ...
        self.model_type = 'ppkg'
        self.pretrain_data = pretrain_data
        self.n_users = data_config['n_users']
        self.n_items = data_config['n_items']
        self.n_entities = data_config['n_entities']
        self.n_relations = data_config['n_relations']
        #self.records = data_config['records']
        self.n_fold = 1
        ...
        self.train_records = data_config['records']
    ...
    def _build_inputs(self):
        ...
        self.users = tensorflow.compat.v1.placeholder(tf.int32, shape=(None,))
        self.pos_items = tensorflow.compat.v1.placeholder(tf.int32, shape=(None,))
        self.neg_items = tensorflow.compat.v1.placeholder(tf.int32, shape=(None,))

        # for knowledge graph modeling (TransD)
        self.A_values = tensorflow.compat.v1.placeholder(tf.float32, shape=[len(self.all_v_list)], name='A_values')
        self.h = tensorflow.compat.v1.placeholder(tf.int32, shape=[None], name='h')
        self.r = tensorflow.compat.v1.placeholder(tf.int32, shape=[None], name='r')
        self.pos_t = tensorflow.compat.v1.placeholder(tf.int32, shape=[None], name='pos_t')
        self.neg_t = tensorflow.compat.v1.placeholder(tf.int32, shape=[None], name='neg_t')

        # dropout: node dropout (adopted on the ego-networks);
        # message dropout (adopted on the convolution operations).
        self.node_dropout = tensorflow.compat.v1.placeholder(tf.float32, shape=[None])
        self.mess_dropout = tensorflow.compat.v1.placeholder(tf.float32, shape=[None])

        #self.keep_prob = tensorflow.compat.v1.placeholder(shape=[None], dtype=tf.float32, name='keep_prob')

        self.records = tensorflow.compat.v1.placeholder(tf.int32, shape=[None,self.input_size ], name='records')
        self.labels = tensorflow.compat.v1.placeholder(tf.int32, shape=[None,self.n_items], name='labels')

        self.lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.n_hidden_units, forget_bias=1.0,
                                                           state_is_tuple=True)
    ...
    def _build_weights(self):
        ...
        self.model = tf.compat.v1.keras.Sequential([
            tf.compat.v1.keras.layers.Flatten(input_shape=[self.input_size, self.kge_dim]),
            tf.compat.v1.keras.layers.Dense(1024, activation='relu'),
            tf.compat.v1.keras.layers.Dense(2048, activation='relu'),
            tf.compat.v1.keras.layers.Dense(1024, activation='relu'),
            tf.compat.v1.keras.layers.Dense(self.n_items, activation='softmax')])
    ...
    def _build_model_phase_I(self):
        if self.alg_type in ['bi',]:
            ...
            print('please check the the alg_type argument, which should be bi,  gcn, or graphsage.')
        ...
        #self.batch_prediction = tf.nn.embedding_lookup(self.weights['entity_embed'], self.records)
        #self.lstm_batch_predictions = self.lstm_layer(all_weights=self._build_weights())
    ...
    def _build_lstm_model(self):
        self.records, self.labels = self._get_lstm_inference(self.records, self.labels)
    '''
    def _get_lstm_inference(self, record):

        records = tf.nn.embedding_lookup(self.weights['entity_embed'], record)

        return records

    def lstm_layer(self):
        #lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.n_hidden_units, forget_bias=1.0,
        #                                                              state_is_tuple=True)
        lstm_cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell=self.lstm_cell, input_keep_prob=1.0,
                                                                       output_keep_prob=1.0)
        lstm_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([lstm_cell for _ in range(self.layer_num)], state_is_tuple=True)
        #__init__state = lstm_cell.zero_state(24, dtype=tf.float32)

        outputs, states = tf.compat.v1.nn.dynamic_rnn(cell=lstm_cell, inputs=self._get_lstm_inference(self.records),
                                                              dtype= float,time_major = False)
        #outputs, states = tf.compat.v1.rnn.static_rnn(cell=lstm_cell, inputs=self._get_lstm_inference(self.records))
        y_pre = self.model(outputs)
        return y_pre
    ...
    def _build_loss_phase_II(self):
        def get_kg_score(h_e, r_e, t_e):
            kg_score = tf.reduce_sum(tf.square((h_e + r_e - t_e)), 1, keepdims=True)
            return kg_score

        pos_kg_score = get_kg_score(self.h_e, self.r_e, self.pos_t_e)
        neg_kg_score = get_kg_score(self.h_e, self.r_e, self.neg_t_e)
    ...
    def get_lstm_loss(self):
        #records = self._get_lstm_inference(self.records)
        pred = self.lstm_layer()
        labels = self.labels
        labels = tf.cast(labels, tf.float32)
        lstm_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=labels))

        return lstm_loss

    def build_loss_lstm_layer(self):
        lstm_loss = self.get_lstm_loss()
        #cross_entropy = -tf.reduce_mean(y_ * tf.log(y_pre))
        lstm_reg_loss = lstm_loss / self.batch_size


        self.lstm_loss3 = lstm_loss
        self.reg_loss3 = self.regs[1] * lstm_reg_loss
        self.loss3 = self.lstm_loss3 + self.reg_loss3

        self.opt3 = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss3)
    ...
    def _create_bi_interaction_embed(self):
        ...
        for k in range(self.n_layers):
            ...
            temp_embed = []
            #A_fold_hat[f] = tf.sparse.to_dense(A_fold_hat[f])
            temp_embed.append(tf.matmul(tf.cast(A, tf.float32), ego_embeddings))
    ...
    def _create_gcn_embed(self):
        ...
        for k in range(self.n_layers):
            ...
            temp_embed = []
            #A_fold_hat[f] = tf.sparse.to_dense(A_fold_hat[f])
            temp_embed.append(tf.matmul(tf.cast(A, tf.float32), pre_embeddings))
    ...
    def train_lstm(self, sess, feed_dict):
        return sess.run([self.opt3, self.loss3, self.lstm_loss3, self.reg_loss3], feed_dict)

    def eval(self, sess, feed_dict):
        #batch_predictions = sess.run(self.batch_predictions, feed_dict)
        with tf.compat.v1.variable_scope("lstm",reuse=tf.compat.v1.AUTO_REUSE):
            batch_predictions = sess.run(self.lstm_layer(), feed_dict)
        #batch_predictions = sess.run(self.batch_predictions, feed_dict)
